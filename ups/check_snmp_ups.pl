...
# Version : 1.1
# Date : April 17 2009
# Author  : J�rgen Vigna <juergen.vigna@wuerth-phoenix.com>
#
...
sub help {
   ...
   print "GPL Licence, (c)2009 J�rgen Vigna\n\n";
   print_usage();
   print <<EOT;
-v, --verbose
...
$status=0;
# Get Output Power per line
$resultat = (Net::SNMP->VERSION gt 4) ?
                  $session->get_table($general_output_power_table)
                : $session->get_table(Baseoid => $general_output_power_table);
if (defined($resultat)) {
    $output_exist=1;
    $o_output_lines=3;
    my $i=1;
    ...
    foreach my $key ( sort keys %$resultat) {
	     if ($o_output_lines < $i) { next; }
            my $power = $$resultat{$key};
            #verb("OID : $key, Desc : $power : $i");
            if ($power < 0) { next; }
            $perfdata .= " power$i=$power;";
            $total_power+=$power;
            $i++;
    }
    $perfdata .=" total_power=$total_power ";
}
$global_status=set_status($status,$global_status);
